#include "includes.h"
    ...
    #define ADC_VREF_VALUE       3000 //ADC参考电压值，单位V    
...
void power_adc_conversion(uint16_t *power_voltage)
{
    ...
    voltage_temp = ((float)adc_value) * ADC_VREF_VALUE / ((float)4096);

    *power_voltage = (uint16_t)(voltage_temp);
